...
func apply_to(player) -> void:
	print("[TRACE] UpgradeData.apply_to | name='%s' effect_type=%s | unlocked_move=%s" % [
		name, EffectType.keys()[effect_type],
		(unlocked_move.move_name if unlocked_move else "null")
	])
	match effect_type:
